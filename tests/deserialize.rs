...
fn bare_nested_field_automatically_rejects_a_fully_omitted_section() {
    ...
    // SecretConfig has a #[required] field of its own, so omitting `[secret]`
    // entirely must be rejected automatically, with no explicit marking needed on
    // `secret` itself. Since `[secret]` is entirely absent, the error is raised right
    // here (naming `secret`, ApiConfig's own field) — it never gets far enough to
    // recurse into SecretConfig::deserialize and name `password` instead (that's
    // `required_field_missing_within_a_present_nested_table_fails`, above).
    let result: Result<ApiConfig, _> = toml::from_str(r#"api_key = "abc123""#);
    let err = result.unwrap_err().to_string();
    assert!(err.contains("required"), "error was: {err}");
...
fn option_wrapped_nested_field_is_exempt_from_automatic_propagation() {
    let cfg: OptionalApiConfig = toml::from_str("").unwrap();
